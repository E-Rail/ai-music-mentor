...
- **语言** —— 简体中文 / English / 跟随系统。首次打开按浏览器语言判断，繁体也归到简体。报告和 AI 导师的回答都跟着走：打开着的报告当场换语言，切换前写的 AI 总结可以一键重写。录音过程中切换不会中断这一遍。
- **详细程度** —— 两种模式长得一样，专业只是多显示一些。标准只说该练什么；专业再加上逐条证据、每个判断的把握、每只手各自的准确度、时间和力度以及两手是否同时落下、力度范围与两手平衡、谱上的断奏、连线、重音和渐强渐弱做到了几处、停顿与回头重弹、这次录音本身的质量（房间底噪、采用与丢弃的音数），以及你实际弹的速度曲线（对照乐谱每处标的速度）。
